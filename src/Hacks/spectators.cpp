...
void Spectatorlist()
{
    ...
    draw->drawstring(x + 30, y + 15, Color::Red(), specfont, spec_title.c_str());
    ...
    for (int playerId : GetObservervators(pEngine->GetLocalPlayer())) {
        ...
        count += 1;
        draw->drawstring(x + 30, y + 30 + (12 * count), Color::Red(), specfont, entityInformation.name);
